...
## Тест данных методов
В браузере в swagger будут доступны 7 HTTP-методов, выполняющих тесты запросов.
...
1. При вызове **GET /Events**. Получаем все мероприятия в формате JSON-объекта(Id, Title, Description, StartAt, EndAt, TotalSeats, AvailbleSeats).

Формат модели события
```
{
  "id": 0,
  "title": string,
...
где
id - целочисленный тип, является идентификатором самого мероприятия и является уникальным
title - строка, является названием мероприятия
description - строка, описание мероприятия
startAt - дата начала мероприятия
endAt - дата окончания мероприятия
totalSeats - целочисленный тип, количество мест на данное мероприятие
availableSeats - целочисленный тип, доступное на данный момент количество мест на данное мероприятие
...
Создается новый ID автоматически. Title должен быть заполнен, Description необязательное поле для заполнения. StartAt и EndAt в формате DateTime. EndAt должен быть позже StatAt. TotalSeats обязательно для заполнения.
...
* TotalSeats обязательное для заполнения
...
В приложении были применены такие примитивы синхронизации:
1. Семафор - асинхронный потокобезопасный примитив. Предназначен для вхождения определенного количества потоков к одному общему ресурсу (В данном сервисе только один поток может входить). Данный примитив необходим для того, чтобы при параллельной обработке множества брони, другие потоки не могли изменить общий ресурс.
2. Конкурентные коллекции - использованы ConcurrentDictionary для хранения ID и объекта события, а также ID и обЪекта брони в формате ключ-значение, ConcurentQueue для хранения очереди ID задач бронирования. Данные коллекции необходимы для потокобезопасной записи и чтения данных при параллельной работе сервиса, т.к. несколько потоков могут одновременно изменить одно и тоже значение, что чревато непредвиденными изменениями 
3. Lock - синхронный потокобезопасный примитив. Предназначен для вхождения только одного потока в критическую секцию. Данный примитив необходим для того, чтобы при параллельном создании брони другие потоки не могли изменить общий ресурс.
