...
2. Конкурентные коллекции - использованы ConcurentDictionary для хранения ID и объекта события, а также ID и обЪекта брони в формате ключ-значение, ConcurentQueue для хранения очереди ID задач бронирования. Данные коллекции необходимы для потокобезопасной записи и чтения данных при параллельной работе сервиса, т.к. несколько потоков могут одновременно изменить одно и тоже значение, что чревато непредвиденными изменениями 
